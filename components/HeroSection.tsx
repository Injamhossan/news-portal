...
export default function HeroSection({ news = [] }: HeroSectionProps) {
  ...
  return (
    <section className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8 mb-8 md:mb-12 font-anek">
      ...
      >
        <Link href={`/news/${mainNews.slug || mainNews._id}`} className="relative group overflow-hidden rounded-xl shadow-sm h-[300px] sm:h-[400px] md:h-[500px] block">
          {/* Dynamic Image */}
      ...
      <motion.div 
        ...
        className="bg-white rounded-xl border border-gray-100 p-4 sm:p-6 shadow-sm h-full overflow-y-auto max-h-[400px] lg:max-h-full"
      >
